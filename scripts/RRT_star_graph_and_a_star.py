...
clean_graph = None #saves the loaded graph to refresh graph after a_star (importanat for #goal>1)
graph_loaded = False
cost = 0
partition_costs = []
xy_graph = np.zeros((0, 2))
max_cost_2_goal = 6 #maximum cost to goal
rad_obs = 0.25 #my min dist to an obs that its valid
rad_search = 6 #search radius vor near vertex
step = 1 #max step size
start = None #start vertex, id = len(graph)
goal = None #goal vertex, id = len(graph)+1
resolution = 0.05 #map param
...
def get_trace():
    ...
    global cost
    cost = 0
    global partition_costs
    partition_costs = []
    for i in range(len(res)-1):
        for neighbor in res[i].neighbors:
            if neighbor[0].id == res[i+1].id:
                cost += neighbor[1]
                partition_costs.append(neighbor[1])
...
def plot_driven_trace(trace, color):
    for i in range(len(trace)-1):
        delta_t = 0.1
        xyp_state = trace[i]
        while not near_to(xyp_state, trace[i+1], rad_obs):
            rpd_state = convert_xyp_2_rpd(xyp_state, trace[i+1])
            v, omega = smooth_control(rpd_state)
            xyp_state = kin_update_xyp(xyp_state, v, omega, delta_t)
...
#loads a img of gazebo slam world1
#not reachable: 205
#wall: 0
#free space: 254
...
def calc_graph(world, start_xyp): #(0, 0, 1.57); (0, 0, 0); (0, 0, -1.57)
    ...
    path = os.path.join(path, 'worlds/graph.obj') #final path
...
def main(world, start_xyp, goal_xyp, load_saved_graph, calc_new_graph):
    ...
    if calc_new_graph:
        calc_graph(world=world, start_xyp=start_xyp)
        clean_graph = graph
        graph_loaded = True
        return True, None, None
    else:
        if not graph_loaded: #is the graph already load? (importanat for #goal>2)
            #init_world
            #path to worlds dir
            path = os.path.dirname(os.path.abspath(__file__))
            path = os.path.dirname(path)
            ...
            clean_graph = pickle.load(file) #load the graph
            lim = len(graph)
    ...
    graph = clean_graph #refresh the graph for the next a_star
    
    trace = calc_trace(start_xyp, goal_xyp)
    
    if trace != None:
        #plot_it(trace)
        return True, trace, cost, partition_costs
    else:
...
#start_xyp=(0, 0, 1.57)
#goal_xyp=(4, -1, 0)
#calc_new_graph = False
#load_saved_graph = True
#
#main(world, start_xyp, goal_xyp, load_saved_graph, calc_new_graph)
...
world6:
    Start: (0, 0, 0), Ziel: (15.0, 10.5, -0.4)
